...
docker build --tag saltyrtc-server .
...
docker run --rm --cpuset-cpus="0" --network saltyrtc --ip="172.19.0.2" --name=saltyrtc --mount type=volume,source=saltyrtc-volume,target=/usr/src/saltyrtc-server/saltyrtc/server/logs saltyrtc-server

# To run the experiment using Docker SDK for Python
# The Python script should automatically run the server and client containers
# so there is no need to call docker run separately.
# But you do need to build the container images first!
